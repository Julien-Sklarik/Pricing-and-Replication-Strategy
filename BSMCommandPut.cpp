...
const std::string BSMCommandPut::getName() const {
  return "Command giving the valuation of a put";
}

const std::string BSMCommandPut::getDescription() const {
  return "This command gives the valutation of a put after entering parameters "
         "like the strike price and the interest rate by using the Black "
         "Scholes Merton equation.";
}

float BSMCommandPut::put(float s, float r, float sigma, float k,
                         float t) // To complete
{
...
bool BSMCommandPut::operator()() {
  ...
  BSMCommandPut::askParameters(parameters);
  ...
  std::cout << "The value of the option is " << put(s, r, sigma, k, t) << std::endl;
  return true;
}